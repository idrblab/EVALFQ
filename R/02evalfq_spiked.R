...
#' @import impute
#' @usage lfqspikedall4(data_s, spiked, assum_a="Y", assum_b="Y",
#' assum_c="Y", Ca="1", Cb="1", Cc="1", Cd="1", Ce="1")
#' @export lfqspikedall4
#' @examples
#' library(EVALFQ)
#' \donttest{data_s <- read.csv(file = "ANPELA-Spiked.INPUT.txt",
#' sep="\t",header = TRUE,stringsAsFactors = FALSE)}
#' \donttest{lfqspikedall4(data_s = data_s, spiked = "ANPELA-Spiked.Protein.txt",
#' assum_a="Y", assum_b="Y", assum_c="Y", Ca="1", Cb="1", Cc="1", Cd="1", Ce="1")}

lfqspikedall4 <- function(data_s, spiked, assum_a="Y", assum_b="Y", assum_c="Y", Ca="1", Cb="1", Cc="1", Cd="1", Ce="1"){
        ...
        for(p in  scaling){
          ...
          for(j in normalization){
            ...
            for(g in 1:8){
              ...
              if( Ca == 1 ){
                #dir.create(paste0("Criteria.A-Precision"))
                #cat(paste("Assessing" , paste(time,".",sep=""), paste(iName[ii],"-",oName[o],"-",pName[p],"-", jName[j], "-",gName[g],sep=""),"Under Criteria A: Precision"),"\n")
              ...
              if( Cd == 1 && length(label) > 6){
                ...
                score <- try(stabel.score(repeats = 200, fold = 2, top = 50))
              ...
              if( Ce == 1 ){
                #dir.create(paste0("Criteria.E-Accuracy"))
                cat(paste("Assessing" , paste(id1,"/",length(newname),":",sep=""), paste(iName[ii],"-", oName[o],"-",pName[p],"-",jName[j], "-",gName[g],sep=""),"Under Criteria E: Accuracy"),"\n")
                ...
                backgound[[paste(ii,o,p,j,g,sep="")]]<-median(backgound_protein,na.rm=TRUE)

                DeviationFC[paste(ii,o,p,j,g,sep="")]<-median(DifferFC,na.rm=TRUE)

                pdf(file=paste("./Criteria.E-Accuracy/Criteria.E_",ii,o,p,j,g,".pdf",sep=""),width = 13,height = 12)
