...
#' @import impute
#' @usage lfqevalueall0(data_q, assum_a="Y", assum_b="Y",
#' assum_c="Y", Ca="1", Cb="1", Cc="1", Cd="1")
#' @export lfqevalueall0
#' @examples
#' library(EVALFQ)
...
#' \donttest{lfqevalueall0(data_q = data_q,
#' assum_a="Y", assum_b="Y", assum_c="Y", Ca="1", Cb="1", Cc="1", Cd="1")}


lfqevalueall0 <- function(data_q, assum_a="Y", assum_b="Y", assum_c="Y", Ca="1", Cb="1", Cc="1", Cd="1"){
          ...
          for(j in normalization){
            ...
            for(g in 1:8){
              ...
              if( Cc == 1 ){
                ...
                breaks<-seq(0,1,0.05)
                sdres<-hist(rots.out$pvalue,breaks=breaks)
              ...
              if( Cd == 1 && length(label) >= 20 ){
                ...
                try(venn(fold = 5, top = 20))
                dev.off()

              }else{
                print("Reproducibility (Criteria.D-Reproducibility) cannot be evaluated, Please Check!")
              }

              #cat(paste("Assessment of", paste(iName[i],"-", oName[o],"-",pName[p],"-",jName[j], "-",gName[g],sep=""),"was completed!"))
